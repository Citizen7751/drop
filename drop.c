#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <string.h>
#include "drop.h"

const char EXIT_KEY = ' ';
const char EMPTY_SPACE = ' ';
const char BORDER = '|';


void set_color (const color_t* pcol, const uint8_t* is_ansi) {
	(*is_ansi) ?
		printf("\033[%u;%um", pcol->ansi.bg, pcol->ansi.fg) :
        printf("\033[48;2;%u;%u;%um\033[38;2;%u;%u;%um",
            pcol->rgb.bgr, pcol->rgb.bgg, pcol->rgb.bgb,
            pcol->rgb.fgr, pcol->rgb.fgg, pcol->rgb.fgb);
}

/*
When the command prompt/terminal is resized, the colors just "splash"
everywhere in a long line, hence the need for actual borders where
all the ansi graphic settings are set back to default and a character
is printed with those settings.
*/
static inline void print_border() {
    printf("\033[0m%c\n", BORDER);
}

void print_colored_line(
    const size_t length,
	const color_t* pcol, uint8_t* is_ansi) {

    set_color(pcol, is_ansi);
    for (size_t i = 0; i < length; i++)
        putchar(EMPTY_SPACE);
    print_border();
}

void print_colored_title(
    const Verse* v,
    const color_t* pcol, uint8_t* is_ansi) {

	set_color(pcol, is_ansi);
    const size_t first_half = (PRINTING_LENGTH - strlen(v->title)) / 2;
    for (size_t i = 0; i < first_half; i++)
        putchar(EMPTY_SPACE);
    printf("%s", v->title);	
	for (size_t i = 0; i < PRINTING_LENGTH - first_half - strlen(v->title); i++)
        putchar(EMPTY_SPACE);
    print_border();
}

void print_colored_text(
    const Verse* v,
	const color_t* pcol, uint8_t* is_ansi) {

	size_t column = 0;
    for (size_t i = 0; v->text[i] != '\0';) {
		set_color(pcol, is_ansi);
        for (column = 0; column <  PRINTING_LENGTH && v->text[i] != '\0'; i++, column++)
            putchar(v->text[i]);
        print_colored_line(PRINTING_LENGTH - column, pcol, is_ansi);
    }
}

void print_verse(uint8_t is_ansi) {
    Verse rand_verse;

    color_t header = {
        {43, 30},
        {151, 139, 37, 70, 50, 10}
    };

    color_t text = {
        {47, 30},
        {102, 67, 14, 255, 255, 255}
    };

    do {
        rand_verse = Bible[rand()%BIBLE_LENGTH];
        print_colored_title(&rand_verse, &header, &is_ansi);
        print_colored_line(PRINTING_LENGTH, &text, &is_ansi);
        print_colored_text(&rand_verse, &text, &is_ansi);
        print_colored_line(PRINTING_LENGTH, &text, &is_ansi);
        print_colored_line(PRINTING_LENGTH, &header, &is_ansi);
    } while(getchar() != EXIT_KEY);
}

int main(int argc, char* argv[]) {
    ANSI_ON;
    srand(time(NULL));

    uint8_t is_ansi = (argv[1] != NULL && !strcmp(argv[1], "-ansi")); 
    print_verse(is_ansi);

    while(getchar()!='\n');
    ANSI_OFF;
    return EXIT_SUCCESS;
}